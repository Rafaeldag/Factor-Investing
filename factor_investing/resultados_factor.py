...
class MakeReportResult():
    ...
    def resultados_moveis(self):
        ...
        self.retorno_1 = self.calcula_retorno_movel(1)
        self.retorno_1_min  = self.retorno_1['retorno_movel'].min()
        self.retorno_1_mean  = self.retorno_1['retorno_movel'].mean()
        self.retorno_21 = self.calcula_retorno_movel(21)
        self.retorno_21_min  = self.retorno_21['retorno_movel'].min()
        self.retorno_21_mean  = self.retorno_21['retorno_movel'].mean()
        self.retorno_63 = self.calcula_retorno_movel(63)
        self.retorno_63_min  = self.retorno_63['retorno_movel'].min()
        self.retorno_63_mean  = self.retorno_63['retorno_movel'].mean()
        self.retorno_126 = self.calcula_retorno_movel(126)
        self.retorno_126_min  = self.retorno_126['retorno_movel'].min()
        self.retorno_126_mean  = self.retorno_126['retorno_movel'].mean()
        self.retorno_252 = self.calcula_retorno_movel(252)
        self.retorno_252_min  = self.retorno_252['retorno_movel'].min()
        self.retorno_252_mean  = self.retorno_252['retorno_movel'].mean()
        self.retorno_504 = self.calcula_retorno_movel(504)
        self.retorno_504_min  = self.retorno_504['retorno_movel'].min()
        self.retorno_504_mean  = self.retorno_504['retorno_movel'].mean()
        self.retorno_756 = self.calcula_retorno_movel(756)
        self.retorno_756_min  = self.retorno_756['retorno_movel'].min()
        self.retorno_756_mean  = self.retorno_756['retorno_movel'].mean()
        self.retorno_1260 = self.calcula_retorno_movel(1260)
        self.retorno_1260_min  = self.retorno_1260['retorno_movel'].min()
        self.retorno_1260_mean  = self.retorno_1260['retorno_movel'].mean()
        self.retorno_2520 = self.calcula_retorno_movel(2520)
        self.retorno_2520_min  = self.retorno_2520['retorno_movel'].min()
        self.retorno_2520_mean  = self.retorno_2520['retorno_movel'].mean()
    
    def calcula_retorno_movel(self, janela):

        df = self.df_trades.copy()
        ibov_df = self.ibov.copy()
        # cdi_df = self.cdi.copy()
        ...
        # cdi_df['retorno_movel_cdi'] = cdi_df['fechamento'].pct_change(janela)
        # cdi_df = cdi_df[['data', 'retorno_movel_cdi']]

        df = pd.merge(df, ibov_df, on = 'data')
        # df = pd.merge(df, cdi_df, on = 'data')

        df['alfa'] = (1 + df['retorno_movel'])/ (1 + df['retorno_movel_ibov']) - 1
        # df['alfa_cdi'] = (1 + df['retorno_movel'])/ (1 + df['retorno_movel_cdi']) - 1

        df = df.dropna()
        df = df[['data', 'retorno_movel', 'alfa']]

        df_calc_percents = df
        df_calc_percents.alfa = df_calc_percents.alfa.astype(float).fillna(0.0)

        percent_periodos_perdendo = len(df_calc_percents[df_calc_percents['retorno_movel'] < 0].index)/len(df_calc_percents.index)
        percent_periodos_perdendo_ibov = len(df_calc_percents[df_calc_percents['alfa'] < 0].index)/len(df_calc_percents.index)
        # percent_periodos_perdendo_cdi = len(df_calc_percents[df_calc_percents['alfa_cdi'] < 0].index)/len(df_calc_percents.index)

        retorno_medio = df_calc_percents['retorno_movel'].mean()

        print(f'Em janelas de {janela} dias, o modelo teve retorno negativo em: {round(percent_periodos_perdendo*100,1)}%')
        print(f'Em janelas de {janela} dias, o modelo teve alfa negativo em: {round(percent_periodos_perdendo_ibov*100,1)}%')
        print(f'Em janelas de {janela} dias, o modelo teve um retorno medio de: {round(retorno_medio*100,1)}%')
        # print(f'Em janelas de {janela} dias, o modelo rendeu menos que o cdi em: {round(percent_periodos_perdendo_cdi*100,1)}%')
        print('------------------')
        return df
    ...
    def fazer_grafico_janelas_moveis(self):
        ...
        self.grafico_retorno_movel(self.retorno_1260, "60M", 'retorno_movel')
        self.grafico_retorno_movel(self.retorno_1260, "60M", 'alfa')
        self.grafico_retorno_movel(self.retorno_2520, "120M", 'retorno_movel')
        self.grafico_retorno_movel(self.retorno_2520, "120M", 'alfa')
