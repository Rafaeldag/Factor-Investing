...
class MakeIndicator():
    ...
    def media_movel_volume_proporcao(self, n_dias_curto, n_dias_longo):
        ...
        volume_medio.to_parquet(f'volume_medio_prop_{n_dias_curto}_{n_dias_longo}.parquet', index = False)
    ...
    def media_movel_volume(self, n_dias):
        ...
        volume_medio = cotacoes[['data', 'ticker', 'valor']]
        volume_medio.to_parquet(f'volume_medio{n_dias}.parquet', index = False)

    
    def cagr_lucro_anos(self, anos):

        df_lucros = pd.read_parquet('LucroLiquido12m.parquet')
        ...
        df_cagr.to_parquet(f'growth_lucro_{anos}.parquet', index = False)

    def cagr_receita_anos(self, anos):

        df_receita = pd.read_parquet('ReceitaLiquida12m.parquet')
        ...
        df_cagr['cagr'] = df_cagr['valor']/df_cagr['valor_x_anos_atras']
        df_cagr = df_cagr[['data', 'ticker', 'cagr']].rename(columns={'cagr': 'valor'})
        df_cagr.to_parquet(f'growth_receita_{anos}.parquet', index = False)
...
if __name__ == "__main__":
    ...
    indicador = MakeIndicator(caminho_dados=r'C:\Users\rafae\OneDrive\Documentos\Bolsa de Valores\Modelos_Quantitativos\base_dados_br')
